...
def valid_puzzle(puzzle: list) -> bool:
    if not isinstance(puzzle, list):  # Check the form of data.
        return False

    if len(puzzle) <= 1:  # Elements>1 in puzzle.
        return False

    for word in puzzle:  # Traverse the list.
        if len(word) != len(puzzle[0]):  # Compare the length of each word.
            return False
    return True  # Ture if equal, false not equal.


def similarity_grouping(data: list) -> list:
    if not isinstance(data, list):  # Check whether the form of data is list.
        return []  # If not, the result is just [].

    result = []  # Set up a list to hold the final result.
    check = {}  # Check whether the word has appeared.

    for word in data:
        if word in check:  # Whether the word has already appeared
            index = check[word]
            result[index].append(word)  # Add it to the same list.
        else:  # Show it has already appeared.
            index = len(result)
            check[word] = index  # Add word and index to check
            result.append([word])  # Add a new sublist to result.
    return result
...
def highest_count_items(data: str) -> list:
    ...
    data = data.split(',')  # Divide the data by the","

    item_counts = {}  # Hold the counts for each data.
    for word in data:
        word = word.strip()
        if word in item_counts:
            item_counts[word] += 1  # Store the counts for each word
        else:
            item_counts[word] = 1

    max_word: int = max(item_counts.values())

    result = []
    count: int
    for item, count in item_counts.items():  # Out put the max count.
        if count == max_word:
...
def valid_char_in_string(popList: list, charSet: list) -> bool:
    ...
    for word in charSet:  # Element in charSet is not 1.
        if len(word) != 1:
            return False

    for string in popList:  # Elements in the popList is in the charSet.
        for word in string:
...
def total_price(unit: int) -> float:
    singleton = unit % 6  # Don't use the suit.
    suit = unit // 6  # The number of the suit.
    price: float = (suit * 5) + (singleton * 1.25)

    if price > 20:  # Use the discount.
        new_price = price * 0.9
        return new_price
    else:
        return price
